...
@section('content')
                ...
                <div class="row m-0">
                    ...
                    <div class="col col-3 border-start">
                        ...
                        <button type="button" data-bs-toggle="tooltip" title="Assign permission/s to Role"
                            class="btn controls-btn">
                            <a data-bs-toggle="modal" data-bs-target="#assignPermissions"
                                data-url="{{ route('get.permissions', [$role->id]) }}"
                                class="assignPermissionsTrigger fa fa-passport warning"></a>
                        </button>
                        <button type="button" data-bs-toggle="tooltip" title="Delete Role" class="btn controls-btn">
                            <a href="{{ route('destroy.role', [$role->id]) }}" class="danger fa fa-trash"></a>
                        </button>
    ...
    <div class="modal fade" id="assignPermissions" tabindex="-1" aria-labelledby="assignPermissionsLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="assignPermissionsLabel">Assign Permissions To Role</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="assign_permissions_form">
                        {{-- href="{{ route('assign.role.to.admin', [$role->id]) }}" --}}
                    </div>
                </div>
            </div>
...
@section('scripts')
    <script>
        ...
        $('.assignPermissionsTrigger').click((e) => {
            const __url = e.target.getAttribute('data-url')
            console.log(__url)
            jQuery.ajax({
                url: __url,
                ...
                success: (data) => {
                    ...
                    $('#assign_permissions_form').html(data)
                },
                error: (err) => {
                    console.log(err)
                }
